...
class TestParseRuleInputToSPAPI:
    @pytest.mark.parametrize("type_", list(RuleType))
    @pytest.mark.parametrize("operator", list(RuleOperator))
    def test_parse_rule_input_to_spapi_model(self, type_: RuleType, operator: RuleOperator) -> None:
        spapi_category = parse_rule_input_to_spapi(RuleCreate(type=type_, operator=operator, value="100"))
